...
$("#btnBajaAsignatura").click(function(){

  var form = $('<form>').addClass('formu form-horizontal');
  form.attr('name', 'formuBajaAsignatura');
  var div = $('<div>').addClass('form-group');
  var label = $('<label>').addClass('control-label col-sm-2');
  label.attr('for', 'id');
  label.append('ID:');
  var div2 = $('<div>').addClass('col-sm-6');
  ...
  var div4 = $('<div>').addClass('col-sm-offset-2 col-sm-10');
  var input2 = $('<input>').addClass('btn btn-primary');
  input2.attr({
    type: 'button',
    name: 'btnEnviarBajaAsignatura',
    value: 'Enviar'
  });
  var input3 = $('<input>').addClass('btn btn-default');
  input3.attr({
    type: 'reset',
    value: 'Resetear campos'
  });
  div4.append(input2);
  div4.append(input3);
  div3.append(div4);
  ...
  var listadoDeCursos = $('#listado');
    ...
    $.post("php/listaCursos.php", null, function(json)
    {
      ...
      {//si hay cursos
        ...
        var input_fin =$('<input type="text" name="fecha_fin" placeholder="2016-05-05" id="fecha_fin_busqueda">');
        ...
        var inputBusqueda =$('<input type="text" name="fecha_inicio" placeholder="2015-05-05" id="fecha_inicio_busqueda">');
          ...
          if (bValido==true) 
          {
              $.post("php/listaCursos.php", null, function(json)
              {
                ...
                {//si hay cursos
                    ...
                    input_fin.attr('id','fecha_fin');
                    listadoDeCursos.find('h3').after('<button id=buscarCurso>Buscar</button>');
                    listadoDeCursos.find('h3').after('<button id=devolverListado>Volver</button>');
                    listadoDeCursos.find('h3').after(input_fin);
                    listadoDeCursos.find('h3').after(label);
                    ...
                    listadoDeCursos.find('h3').after(label);
                    $('#buscarCurso').click(buscarCursos);
                    $('#devolverListado').click(mostrarListadoCursos);
...
$('#fechaNac_Alumno').datepicker({ dateFormat: 'yy-mm-dd' });
$('#fechaNac_Profesor').datepicker({ dateFormat: 'yy-mm-dd' });
$('#fecha_ini_Curso').datepicker({ dateFormat: 'yy-mm-dd' });
$('#fecha_fin_Curso').datepicker({ dateFormat: 'yy-mm-dd' });
...
function mostrarListadoProfesores(){
    ...
    localStorage['profesores'] = profesores;
...
function mostrarListadoAsignaturas(){
	...
	listadoDeCursos.load('php/listadoAsignaturas.php', null, function(respuesta){
		...
		oDlgGestionListaAsignaturas.dialog({ title: "Listado de Asignaturas",
                                          width: 900,
                                          height:500,
                                        }); 
...
function validarAltaCurso(){
    ...
    if(bValido==false){
        ...
        var sLocalizacion = oForm.localizacion_curso.value.trim();
        //Creamos Curso
